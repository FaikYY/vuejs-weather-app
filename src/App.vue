...
<template>
  ...
  >
    <main>
      <SearchBar />
      <WeatherInfo />
      <a href="https://vuejs-profile-card.netlify.app/" id="watermark">Created by Faik Yesilyaprak</a>
    </main>
  </div>
</template>
  ...
  computed: {
    ...
    checkWeatherForBackground() {
      if (this.$store.state.weather.weather[0].main == "Clear") {
        ...
        this.$store.state.weather.weather[0].main == "Clouds" &&
        Math.round(this.$store.state.weather.main.temp) >= 16
      ) {
